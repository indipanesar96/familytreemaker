...
$ python familytreemaker.py -a "王灿文" -v2 -o LouisXIVfamily.dot LouisXIVfamily1.txt && dot -Tsvg -o LouisXIVfamily.svg LouisXIVfamily.dot

will generate the tree from the infos in LouisXIVfamily1.txt, starting from
...
class Family:
    ...
    def __init__(self, infolevel=0, outmode=0, gender=["M", "F"]):
...
def parse_arguments():
    ...
    parser.add_argument('-g', dest='gender', action="store", default="M,F",
                        help='customized gender string, for example: "M,F" or "M,F"')
    parser.add_argument('-v', dest='infolevel', action="store", default=0, type=int,
                        help='Information level (0/1/2) to output. (' +
                             '0 - only name and surname will be output; ' +
    ...
    if outmode == 0:
        ...
        with open(args.outfile, "w", encoding="utf-8") as f:
            for i in family.output_descending_tree(ancestor):
                f.write(str(i) + "\n")
    # to ensure the .dot file is written before parsing it with dot
    # time.sleep(3)
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    # python familytreemaker.py -a "Gurdev S Panesar" -v0 -o Panesarfamily.dot PanesarFamilyOldFormat.txt | dot -Tpng -o Panesar.png Panesarfamily.dot